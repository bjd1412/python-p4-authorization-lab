...
class MemberOnlyIndex(Resource):
    ...
    def get(self):
        if session['user_id']:
            members = [article.to_dict() for article in Article.query.filter(Article.is_member_only == True).all()]
...
class MemberOnlyArticle(Resource):
    ...
    def get(self, id):
        if session['user_id']:
            members = Article.query.filter(Article.id == id).first()
            return make_response(members.to_dict(), 200)
        else:
            return make_response({'Error':'Unauthorized'}, 401)
